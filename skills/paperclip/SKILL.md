...
- **Reading a resolved-interaction wake.** When you wake on an interaction resolution, the wake payload carries `interaction outcome` (accepted/rejected/…) and, for declines, `interaction reason`. **Read `result.reason` (the `interaction reason` line) before doing anything else.** If the outcome is `rejected`, do not recreate the same card — address the stated reason first. If you no longer need a different open request you created on this issue, cancel/supersede it (`POST …/interactions/{id}/cancel`) instead of leaving it pending.
- **Pending-accumulation guard.** Creating a new `request_confirmation`/`request_checkbox_confirmation` as an agent automatically expires your own prior pending confirmations on the same issue that share the new target (or are likewise targetless), resolving them with `outcome: "superseded_by_new_request"`. Rely on this rather than stacking many open cards, but still cancel pending requests you have abandoned so the board is not asked to decide stale questions.
